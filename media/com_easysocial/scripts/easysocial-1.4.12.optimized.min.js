...
return b.pollExpiry().trigger("datetimeExport",[d]),b.pollItem().length>0&&(a.each(b.pollItem(),function(b,d){var e=a(d);""!=e.val()&&c.push(e.val())}),d.items=c),d},"{voteOption} change":function(c){var e=a(c),f=e.is(":checked")?!0:!1,g=e.data("id"),h="";f?(b.updateVote(g,"vote",c),h="vote"):(b.updateVote(g,"unvote",c),h="unvote"),f&&!b.options.isMultiple&&a.each(b.voteOption(),function(c,d){var e=a(d);e.data("id")!=g&&e.is(":checked")&&(selectItem=e.data("id"),e.prop("checked",!1),b.updateVote(selectItem,"unvote",e),h="changeVote")
}),b.notify(h,g)},notify:function(a,c){EasySocial.ajax("site/controllers/polls/notify",{id:b.options.id,itemId:c,action:a}).done(function(){})},"{viewVoterBtn} click":function(c){var e=a(c).closest("[data-vote-item]"),f=a(e).data("id");EasySocial.ajax("site/views/polls/voters",{id:b.options.id,itemid:f}).done(function(b){if(""!=a.trim(b)){var c="[data-poll-voters-"+f+"]";a(c).html(b).removeClass("hide")}}).fail(function(){})},updateVote:function(c,d,e){EasySocial.ajax("site/controllers/polls/vote",{id:b.options.id,itemId:c,act:d}).done(function(){var g=a(e).closest("[data-vote-item]"),h=a(g).data("count");
"vote"==d?h+=1:"unvote"==d&&(h-=1),a(g).data("count",h),b.updateProgressBar()}).fail(function(a){b.voteNotice().removeClass("hide").text(a.message)})},"{editBtn} click":function(){},"{resultBtn} click":function(){b.showResults()},"{voteBtn} click":function(){var e=[];a.each(b.voteOption(),function(b,c){var d=a(c);d.is(":checked")&&e.push(d.data("id"))}),0!=e.length&&(b.voteBtn().hide(),b.showResults(),EasySocial.ajax("site/controllers/polls/vote",{id:b.options.id,items:e}).done(function(a,c){b.updateProgressBar(c),b.viewBtn().removeClass("hide"),b.voteNotice().removeClass("hide").addClass("alert-success").text(a).delay(5e3).fadeOut()
}).fail(function(a){b.voteNotice().removeClass("hide").text(a.message)}))}}}),EasySocial.Controller("Polls.Datetime",{defaultOptions:{"{picker}":"[data-picker]","{toggle}":"[data-picker-toggle]","{datetime}":"[data-datetime]"}},function(b){return{init:function(){var d=new a.moment,e=(new Date).getFullYear()+10,f="DD-MM-YYYY hh:mm A";d.date(d.date()-1),b.picker()._datetimepicker({component:"es",useCurrent:!1,format:f,minDate:d,maxDate:new a.moment({y:e}),icons:{time:"glyphicon glyphicon-time",date:"glyphicon glyphicon-calendar",up:"glyphicon glyphicon-chevron-up",down:"glyphicon glyphicon-chevron-down"},sideBySide:!1,pickTime:1,minuteStepping:1,language:c});
...
})}}}),b.resolve()})}),EasySocial.module("site/toolbar/profile",function(){var b=this;EasySocial.require().library("popbox").done(function(a){EasySocial.Controller("Toolbar.Profile",{defaultOptions:{"{dropdown}":"[data-toolbar-profile-dropdown]","{dropdownToggle}":"[data-toolbar-toggle]"}},function(){return{init:function(){},"{self} popboxActivate":function(b,c,d){a(d.tooltip).implement(EasySocial.Controller.Toolbar.Profile.Logout)},"{dropdownToggle} click":function(){a(".popbox-notifications").hide()}}}),EasySocial.Controller("Toolbar.Profile.Logout",{defaultOptions:{"{logoutForm}":"[data-toolbar-logout-form]","{logoutButton}":"[data-toolbar-logout-button]"}},function(a){return{logout:function(){a.logoutForm().submit()
