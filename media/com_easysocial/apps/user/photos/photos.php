...
class SocialUserAppPhotos extends SocialAppItem
{
	...
	public function prepareGroupPhotoStream(SocialStreamItem &$item, $privacy, $includePrivacy = true, $useAlbum = false)
	{
		...
		$item->content = parent::display('streams/group/' . $verb . '.content');

		// This is only applied to photo that has been shared from story panel
		if ($verb == 'share') {
			$item->preview = parent::display('streams/group/' . $verb . '.preview');
		}
