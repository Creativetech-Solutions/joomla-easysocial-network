...
class SocialUserAppAudiosHookNotificationComments
{
	...
	public function execute($item, $audio)
    {
        ...
        if ($audio->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER ) {
            ...
            $item->title    = JText::sprintf('APP_USER_AUDIOS_USER_COMMENTED_ON_YOUR_AUDIO' . $plurality, $names );

            return $item;
        }

        if ($audio->user_id == $item->actor_id && count($users) == 1) {
            $item->title = JText::sprintf('APP_USER_AUDIOS_OWNER_COMMENTED_ON_AUDIO' . FD::user($audio->user_id)->getGenderLang(), $names);

            return $item;
        }

        // This is for 3rd party viewers
        $item->title    = JText::sprintf('APP_USER_AUDIOS_USER_COMMENTED_ON_USERS_AUDIO' . $plurality, $names , FD::user( $audio->user_id )->getName() );
