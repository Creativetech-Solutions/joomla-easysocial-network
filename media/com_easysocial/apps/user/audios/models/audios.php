...
class AudiosModel extends EasySocialModel {
    ...
    public function getTable($type = 'Audio') {

        $audio = new SocialAppsController('user', ES_SOCIAL_TYPE_AUDIOS);
        $table = $audio->getTable($type);
    ...
    public function getAudios($options = array()) {
        ...
        $likeCountColumn = "(select count(1) from `#__social_likes` as exb where exb.uid = a.id and exb.type = 'audio.user.create') as likes";
        $commentCountColumn = "(select count(1) from `#__social_comments` as exb where exb.uid = a.id and exb.element = 'audio.user.create') as totalcomments";
        ...
        $aid = $this->normalize($options, 'aid', false);
...
//        exit;
        ...
        foreach ($result as $row) {
            ...
            $audio->load($row->id);
            $category = $this->getTable('AudioCategory');
            $category->load($audio->category_id);
    ...
    public function getDefaultCategory() {
        ...
        $sql->select('#__social_audios_categories');
        $sql->where('default', 1);
        ...
        $category = ES::table('AudioCategory');
    ...
    public function getCategories($options = array()) {
        ...
        $query = array();
        $query[] = 'SELECT a.* FROM ' . $db->qn('#__social_audios_categories') . ' AS a';
...
//            $query[] = 'LEFT JOIN ' . $db->qn('#__social_audios_categories_access') . ' AS b';
...
//            $query[] = 'OR';
//            $query[] = '(a.' . $db->qn('id') . ' NOT IN (SELECT `category_id` FROM `#__social_audios_categories_access`))';
//            $query[] = ')';
//        }
        // Ensure that the audios are published
        ...
        if ($aid) {
            $query[] = 'AND ' . $db->qn('id') . '=' . $db->Quote($aid);
        }
...
//    private function getCategoryRecursive($cats, &$newcats){
//        foreach($cats as $cat){
//            if($cat->parent_id==0){
//                $newcats[] = $cat;
//            }
...
//    }
    public function getCategoryListHtml($selected='') {
        ...
        //genericlist($data, $name, $attribs = null, $optKey = 'value', $optText = 'text', $selected = null, $idtag = false,
