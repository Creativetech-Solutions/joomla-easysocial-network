...
<?php
if (JRequest::getVar('subview') == 'settings') {
    echo $this->output('site/profile/default.edit.profile-settings');
} else {
    ?>
        ...
        <div class="es-sidebar" id="es-sidebar" data-sidebar>
            ...
            <div class="es-widget es-widget-borderless">
                ...
                <ul class="newsfeed-sidebar">
                    <li class="settings">
                        <a href="<?php echo JRoute::_('index.php?option=com_easysocial&view=profile&layout=edit&subview=settings') ?>">
                            <i class="fa fa-cog"></i>
                            <?php echo JText::_('COM_EASYSOCIAL_SETTINGS_BUTTON'); ?>
                        </a>
                        <?php /* ?><a data-toggle="collapse" data-target="#collapseSettings" aria-expanded="false" href="javascript::void(0)">
                                         ...
                                         <?php if ($step->fields) { ?>
                                             <?php foreach ($step->fields as $field) { ?>
                                                 ...
                                                 if (!empty($field->output)) {
                                                     if ($field->element !== 'joomla_password' && $field->element !== 'joomla_email' && $field->element !== 'switchprofile') {
                                                         ?>
                                                    <div data-profile-edit-fields-item data-element="<?php echo $field->element; ?>" data-id="<?php echo $field->id; ?>" class="container_<?php echo $field->unique_key; ?> div_<?php echo $field->element; ?>" data-required="<?php echo $field->required; ?>" data-fieldname="<?php echo SOCIAL_FIELDS_PREFIX . $field->id; ?>">
    ...
    <?php } ?>
                        ...
                        <div class="pull-right">
                            ...
                            <?php if ($this->my->hasCommunityAccess()) { ?>
                                <button type="button" class="btn btn-sm btn-es-primary save-next" data-profile-fields-save-close><?php echo JText::_('COM_EASYSOCIAL_SAVE_AND_NEXT_BUTTON'); ?></button>
                                <button type="button" class="btn btn-sm btn-es-primary save-exit" style="display:none" data-profile-fields-save-close><?php echo JText::_('COM_EASYSOCIAL_SAVE_AND_EXIT_BUTTON'); ?></button>
    <?php } ?>
                        </div>
                    </div>
                    ...
                    <input type="hidden" name="all_steps" id="all_steps_input" value="<?php echo $steps[count($steps) - 1]->id; ?>" />
                    <input type="hidden" name="current_step" id="current_step_input" value="<?php echo $steps[0]->id ?>" />
