...
<div class="es-streams <?php echo ( count($streams) == 0 ) ? ' no-stream' : ''; ?>"
    ...
    <ul class="es-stream-list <?php echo ($streams && is_array($streams)) ? '' : ' is-empty'; ?>"
        data-stream-list>
        <li id="postbox" class="streamItem">
            <?php if (!empty($story)) {
                echo $story->html();
            } ?>
        ...
        <?php if (($streams && is_array($streams)) || (isset($stickies) && $stickies && is_array($stickies))) { ?>
            ...
            <?php if ($streams && is_array($streams)) { ?>
                <?php foreach ($streams as $stream) { 
                    //abrar temporarily hiding the profiles posts
                    if($stream->context=='profiles')                        continue;
                   // print_r($stream);exit;
        ...
        <?php } ?>
            ...
            <?php if ($this->config->get('stream.pagination.style') == 'loadmore') { ?>
                    ...
                    <?php if ($isguest && isset($nextlimit)) { ?>
                        <?php if (FD::user()->id != 0) { ?>
                        <li class="pagination streamPage" data-stream-pagination-guest data-nextlimit="<?php echo $nextlimit; ?>" data-context="<?php echo $this->html('string.escape', $context); ?>">
                <?php if ($nextlimit) { ?>
            ...
            <?php } ?>
        <?php } else { ?>
                    <li class="pagination streamPage"
