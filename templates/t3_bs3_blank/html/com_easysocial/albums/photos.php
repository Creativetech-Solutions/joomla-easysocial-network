		...
		<?php if (!empty($photos)) { ?>
			<?php foreach ($photos as $photo) { ?>
				 <?php if ($options['layout'] != 'dialog') {
                 	echo FD::photo($photo->uid, $photo->type, $photo)->renderItem(array('photoItem' => $options['photoItem'], 'layout'=> 'item','template' => 'site/photos/albums','openInPopup' => 0)); 
                 }else{
                   echo FD::photo($photo->uid, $photo->type, $photo)->renderItem(array('photoItem' => $options['photoItem'], 'layout'=> 'dialog','template' => 'site/photos/item','openInPopup' => 0));   
                 }?>
			<?php } ?>
		<?php } ?>
	</div>
