...
class AudiosControllerAudios extends SocialAppsController {
    ...
    public function unfeature($id){
        ...
        $query
            ->select('id')
            ->from($db->quoteName('#__social_audios'))
            ->where($db->qn('featured') .' = '.$db->q(1).' AND '.$db->qn('user_id').' = '.JFactory::getUser()->id);
        if($res = $db->setQuery($query)->loadResult()){
            if($res == $id){
                return;
            }else{
                $query->clear();
                $query->update($db->quoteName('#__social_audios'))->set($db->quoteName('featured').' = 0')->where($db->quoteName('id').' = '.$res);
                $db->setQuery($query);
                $db->execute();
                return;
            }
