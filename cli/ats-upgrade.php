...
class AppAtsupgrade extends JApplicationCli
{
	/**
	...
	public function execute()
	{
		...
		require_once JPATH_ADMINISTRATOR . '/components/com_ats/version.php';
		...
		echo <<<ENDBLOCK
Akeeba Ticket System Updater CLI $version ($date)
...
ENDBLOCK;
		...
		$jlang->load('com_ats', JPATH_ADMINISTRATOR, 'en-GB', true);

        $this->out("Checking for new versions");

        $container = \FOF30\Container\Container::getInstance('com_ats');

        /** @var \Akeeba\TicketSystem\Admin\Model\Updates $updateModel */
...
$app = JApplicationCli::getInstance('AppAtsupgrade');
