...
class JMSlide extends stdClass {
  ...
  public function loadProduct($id) {
    ...
    $usergroups = JFactory::getUser()->groups;
    $usergroups[] = "1";
    // fix hikashop 2.0.4.1
    if (in_array($product->product_access, $usergroups) || ($product->product_access=="all"))
    {
	    if ($product) {
	      $this->title = $product->product_name;
	      $image_source = $this->params->get('jmslideshow_image_source', 0);
	      if (empty($image_source)) {
	        $this->image = JPATH_SITE . DS . 'images' . DS . 'com_hikashop' . DS . 'upload' . DS . $product->file_path;
	      } else {
  ...
  function getMainImage() {
    ...
    if($this->k2){ // fix k2
      $date = md5($this->k2->modified);
      $basename = $width . 'x' . $height . '_' . $style . '_'. $date . '_' . $file['basename'];
    }
    ...
    $link =  JM_SLIDESHOW_IMAGE_PATH . '/' . $safe_name;
    return $link;
  }
