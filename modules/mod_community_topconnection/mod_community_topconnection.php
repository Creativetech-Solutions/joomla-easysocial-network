...
defined( '_JEXEC' ) or die( 'Unauthorized Access' );
   ...
   // $document->addStyleSheet(JURI::root(true) . '/modules/mod_community_TopConnection/style.css');

	$user = CFactory::getUser();

    $modTopConnectionHelper = new modCommunityTopConnection();
  
    $users = $modTopConnectionHelper->gettopmember($params);
    $tot_user = $modTopConnectionHelper->gettotalmember($params);
...
$path = JModuleHelper::getLayoutPath('mod_community_topconnection', $layout);
if (file_exists($path)) {
	require($path);
}
