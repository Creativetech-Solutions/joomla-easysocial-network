	...
	<div class="allicontopprofle">
		...
		<ul class="user_login_menus">	
   			<li class="user_profile_pic">
   				<a href="javascript:void(0);" onclick="profatch1();">
   				
		            <?php 
			            if($userdetail->profilePicture!='')
			            { 
			              if(file_exists(JPATH_ROOT.'/images/watermarks/'.$userdetail->profilePicture))
			              {
			                 $userpic=$userdetail->profilePicture;
			              }
			              else 
			              {
		            ...
		            ?>
           			 <img class="fa-user" src="<?php echo $image_path_user.$userpic; ?>" alt="<?php echo $user->name; ?>" />
           			 <?php 
           			 	if($userdetail->memberType==1)
           			 	{
           			 		if($userdetail->artistname != '')
           			 		{
           			 			echo '<label title="'.$userdetail->artistname.'">'.$userdetail->artistname.'</label><b>...</b>';
           			 		}
           			 		else if($userdetail->firstName != '')
           			 		{
           			 			echo '<label title="'.$userdetail->firstName.' '.$userdetail->lastName.'">'.$userdetail->firstName.' '.$userdetail->lastName.'</label><b>...</b>';
           			 		}
           			 		else
           			 		{
           			 			echo '<label title="'.$user->name.'">'.$user->name.'</label><b>...</b>';
           			 		}
           			 	}
           			 	...
           			 	{	
           			 		...
           			 		{
           			 			echo '<label title="'.$userdetail->businessname.'">'.$userdetail->businessname.'</label><b>...</b>';
           			 		}
           			 		else if($userdetail->firstName != '')
           			 		{
           			 			echo '<label title="'.$userdetail->firstName.' '.$userdetail->lastName.'">'.$userdetail->firstName.' '.$userdetail->lastName.'</label><b>...</b>';
	    ...
	    <div class="profilediv" id="prodash11" style="display:none;">
	    	...
	    	<ul class="user_profile_ul">
		   		<li class="profile_icon"> 
		   			<a href="<?php echo JRoute::_('index.php?option=com_community&view=profile&newsfeed=1');?>"> <i class="fa fa-user">&nbsp;</i> <?php echo JText::_( 'MOD_JELOGIN_MY_PROFILE'); ?></a></li>
                ...
                <li><a href="<?php echo JRoute::_('index.php?option=com_community&view=inbox');?>"><i aria-hidden="true" class="fa fa-envelope">&nbsp;</i> Messages</a></li>
                
                <li><a href="<?php echo JRoute::_('index.php?option=com_community&view=profile&task=accountsetting');?>"><i aria-hidden="true" class="fa fa-cog">&nbsp;</i>Account Settings</a></li>
                <li><a href="#"><i aria-hidden="true" class="fa fa-question-circle">&nbsp;</i> Help ?</a></li>
                
               <li><a href="#"><i aria-hidden="true" class="fa fa-star">&nbsp;</i> Upgrade to Premium</a></li>
