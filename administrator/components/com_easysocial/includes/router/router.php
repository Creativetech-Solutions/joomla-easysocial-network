...
abstract class SocialRouterAdapter
{
    ...
    public function route($options = array(), $xhtml = true)
    {
        ...
        if (!$menuId && $view == 'albums' && !$layout && $uId && $uType == 'user') {
            $menuId = FRoute::getItemId($view, $layout, $uId, $uType, true, true);

            if (! $menuId) {
                $menuId = FRoute::getItemId($view, '', '', '', true, true);
            }
        }
        ...
        if ($menuId) {
            ...
            if ($menu) {
                ...
                if (isset($tmpQuery['view']) && ($tmpQuery['view'] == 'albums' || $tmpQuery['view'] == 'videos') && isset($tmpQuery['uid']) && isset($tmpQuery['id'])) {
                    unset($tmpQuery['uid']);
