	...
	public function format( $items , $context = 'all', $viewer = null, $loadCoreAction = true, $defaultEvent = 'onPrepareStream' , $options = array() )
	{
		...
		foreach ($items as &$row) {
			...
			if ($streamItem->display != SOCIAL_STREAM_DISPLAY_MINI) {
				...
				if ($streamItem->comments && !($streamItem->context == 'photos' && $streamItem->verb != 'share' && count($streamItem->contextIds) > 1)) {
				...
				if ($streamItem->likes && !($streamItem->context == 'photos' && $streamItem->verb != 'share' && count($streamItem->contextIds) > 1)) {
