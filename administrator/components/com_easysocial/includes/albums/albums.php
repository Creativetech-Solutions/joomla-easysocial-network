...
class SocialAlbums extends JObject
{
	...
	public function renderItem($options = array())
	{
		...
		if (! $viewAllMode) {
			// Get album likes
			$likes = FD::likes($album->id, SOCIAL_TYPE_ALBUM , 'create', $albumGroup );

			// Get album shares
			$shares = FD::get('Repost', $album->id, SOCIAL_TYPE_ALBUM , $albumGroup, $albumGroupId, $albumGroupType);

			// Get album comments
			$comments = FD::comments($album->id, SOCIAL_TYPE_ALBUM , 'create', $albumGroup , array('url' => $album->getPermalink()));
