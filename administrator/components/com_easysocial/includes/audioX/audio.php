...
class SocialAudio extends EasySocial
{
	...
	{
		$tag = ES::tag($this->table->id, 'audio');
	...
	public function insertPrivacy($privacy)
	{
		...
		$privacyLib->add($privacy->rule, $this->table->id, 'audio', $privacy->value, null, $privacy->custom);
	}
	...
	{
		$container = 'media/com_easysocial/audios';
	...
	public function getLocation()
	{
		...
		if (is_null($location)) {
			$location = ES::location($this->table->id, 'audio');
		}
	...
	public function getLikes($verb = '', $streamId = '')
	{
		...
		$likes = ES::likes();
		$likes->get($this->table->id, 'audios', $verb, $this->type, $streamId);
	...
	public function getComments($verb = '', $streamId = '')
	{
		...
		$comments = ES::comments($this->table->id, 'audios', $verb, $this->type, array(), $streamId);
	...
	public function getPrivacy()
	{
		...
		if (is_null($privacy)) {
			$privacy = ES::privacy($this->id, 'audios');
		}
	...
	public function getPrivacyButton()
	{
		...
		$button = $privacy->form($this->table->id, 'audios', $this->table->uid, 'audios.view', false, $streamId);
	...
	public function getVideoStreamId($videoId, $verb)
	{
		...
		$sql->column('a.uid');
		$sql->where('a.context_type', 'audios');
	...
	public function getTags()
	{
		...
		$model = ES::model('Tags');
		$tags = $model->getTags($this->table->id, 'audio');
	...
	public function createStream($verb, $privacy = '')
	{
		...
		// Set the context
		$template->setContext($this->table->id, 'audios');
	...
	public function removeStream($verb, $actorId = '')
	{
		...
		$result = $stream->delete($this->table->id, 'audios', $actorId, $verb);
	...
	public function delete()
	{
		...
		$likes = ES::likes();
		$likes->delete($this->id, 'audios', 'create');
