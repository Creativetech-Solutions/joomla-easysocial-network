...
class ESR
{
    ...
    public static function getMenuLink($menuId)
    {
        ...
        if ($router->getMode() == JROUTER_MODE_SEF && ($menuItem->home != true)) {
     ...
     */
    public static function getMenus( $view , $layout = null, $id = null, $useUID = false, $useExact = true)
    {
        static $menus       = null;
        ...
        if (!isset($selection[ $key ])) {
            ...
            if (isset($menus[ $view ]) && isset($menus[ $view ]) && (is_null($layout) || empty($layout)) && isset($menus[$view][0]) && $id) {
            ...
            if (isset($menus[ $view ]) && isset($menus[ $view ]) && (is_null($layout) || empty($layout)) && (is_null($id) || empty($id))) {

                $viewMenu    = isset( $menus[$view][0] ) ? $menus[$view][0] : false;
                ...
                if ($useExact) {
                   ...
                   if ($viewMenu) {
                        ...
                        $tSegments = get_object_vars($viewMenu[0]->segments);
     ...
     * @return
     */
    public static function getItemId($view, $layout = '', $id = '', $type = false, $useDefault = true, $useExact = false)
    {
        static $views   = array();
        ...
        if (!isset($views[$key])) {
            ...
            if ($view == 'albums' && !$layout && $type == 'user' && $id) {
                $menus = ESR::getMenus($view, '', $id, 'userid', $useExact);
                ...
                if (!$menuId) {
                    $menus = ESR::getMenus($view, '', 0, '', $useExact);
                }
            ...
            // at this point, if there is still no menu item found, and the useExact flag is on, just return false
            if (!$menuId && $useExact) {
                return false;
            }
