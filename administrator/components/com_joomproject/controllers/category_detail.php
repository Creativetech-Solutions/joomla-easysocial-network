...
class category_detailController extends JControllerLegacy {
	...
	//===================================================================================================	
		...
		$insertid = $model->store ( $post,'category');
		...
		if ($insertid) 
		{
				...
				for($i=0;$i<count($skilllistid);$i++)
				{
					...
					$model->store ( $skil,'category_detail');
				}
...
function deleteskill() {
		...
		$query="DELETE FROM ".$this->_table_prefix."category WHERE id=".$skid;
		
		
		$db->setQuery($query);
		if(!$db->Query())
		{
			$msg = JText::_ ( 'CATEGORY_DELETED_FAIL' );
			$this->setRedirect ( 'index.php?option='.$option.'&view=category_detail',$msg );
