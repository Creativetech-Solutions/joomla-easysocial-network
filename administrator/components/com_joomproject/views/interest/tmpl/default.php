...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?tmpl=component&option='.$option.'&view=interest&task=saveOrderAjax';
...
<!---------------------------------------- End Ordering Script ------------------------- interest_detail ----------------->
...
var form = document.adminForm;
   if (pressbutton)
    {form.task.value=pressbutton;}
	 ...
	 {		 
	  form.view.value="interest_detail";
	 }
	try {
		form.onsubmit();
		}
...
<div id="editcell">
	...
	<thead>
		<tr>
			...
			<th class="hidden-phone"  width="10%">
				<?php echo JHTML::_('grid.sort', 'INTEREST_TITLE', 'int_title', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
			</th>
	...
	<?php
	$k = 0;
	// echo '<pre>'; print_r($this->interest); exit;
	for ($i=0, $n=count( $this->interest ); $i < $n; $i++)
	{
	
		$row = &$this->interest[$i];
        $row->id = $row->id;
		$link 	= JRoute::_( 'index.php?option='.$option.'&view=interest_detail&task=edit&cid[]='. $row->id );
		...
		<tr class="row<?php echo $i % 2; ?>" itemid="<?php echo $row->ordering;?>">
			...
			<td>
					<a href="<?php echo $link; ?>" title="<?php echo JText::_( 'EDIT_TAG' ); ?>"><?php  echo $row->int_title; ?></a>
			</td>
...
<input type="hidden" name="view" value="interest" />
