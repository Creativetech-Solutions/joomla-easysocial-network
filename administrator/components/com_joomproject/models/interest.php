...
class interestModelinterest extends JModelLegacy
{
	var $_data = null;
	var $_total = null;
	...
	function _buildQuery()
	{
		...
		$query = ' SELECT * FROM '.$this->_table_prefix.'interest ORDER BY id';
		return $query;
	}
	...
	function getparent($id)
	{
		...
		$query = 'SELECT * FROM '.$this->_table_prefix.'interest WHERE id='.$id;
		...
		function saveorder(){
			...
			$row = JTable::getInstance('interest', 'Table');
			$groupings = array();
			for ($i = 0; $i < $total; $i++){
				$row->load((int)$cid[$i]);
				$groupings[] = $row->name;
			...
			$option= JRequest::getVar('option',com_joomproject, 'request', 'string');
