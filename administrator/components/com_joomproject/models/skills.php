...
class skillsModelskills extends JModelLegacy
{
	var $_data = null;
	var $_total = null;
	...
	function _buildQuery()
	{
		...
		$query = ' SELECT * FROM '.$this->_table_prefix.'skills ORDER BY id';
		return $query;
	}
	...
	function getparent($id)
	{
		...
		$query = 'SELECT * FROM '.$this->_table_prefix.'skills WHERE id='.$id;
		...
		function saveorder(){
			...
			$row = JTable::getInstance('skills', 'Table');
			$groupings = array();
			for ($i = 0; $i < $total; $i++){
				$row->load((int)$cid[$i]);
				$groupings[] = $row->name;
