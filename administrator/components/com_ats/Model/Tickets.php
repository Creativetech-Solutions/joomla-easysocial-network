...
class Tickets extends DefaultDataModel
{
    ...
    /**
     * When false: ATS checks the permissions of the currently logged in user when saving. When true: ATS checks the
     * permissions of the user specified in the created_by field  when saving.
     *
     * @var   bool
     */
    ...
    /**
     * When true ATS will completely ignore user permissions when saving a ticket. This has precedence over the
     * $overrideuser flag above.
     *
     * @var   bool
     */
    ...
    public function check()
    {
        ...
        // If $noPermissionsCheck is set we won't perform any ACL privilege checks
        if (!self::$noPermissionsCheck)
        {
            if (self::$overrideuser || \JFactory::getSession()->get('ticket.overrideuser', 0, 'com_ats'))
            {
                // If we are told to override the user for permissions checking let's do so
                $userid = $this->created_by;
            }
            else
            {
                // Otherwise we set the user ID to null, meaning that we will be checking the permissions of the current
                // logged in user.
                $userid = null;
            }

            // Get the ACL privileges for the specified user and the ticket's category
            $aclPrivileges = Permissions::getAclPrivileges($this->catid, $userid);

            // Make sure the user has the Create privilege for this category
            $this->assert($aclPrivileges['core.create'], JText::_('COM_ATS_TICKETS_ERR_CATNOAUTH'));
        }
    ...
    public function isValid()
    {
        ...
        {
            $jResponse = $platform->runPlugins('onAtsValidate', array($custom, $category));
