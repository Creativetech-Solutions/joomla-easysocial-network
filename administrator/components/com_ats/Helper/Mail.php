...
class Mail
{
	...
	{
        $textOnly = ComponentParams::getParam('text_version', 0);
		$header   = $this->connector->headerInfo($this->connection, $mid);
		...
		$message = array(
			...
			'ticketid'    => $ticketid,
			'parts'       => array(),
            'body'        => '[EMAIL CONTAINS NO MESSAGE BODY]'
		);

		// Let's build a stack of all the available texts, first HTML otherwise fallback to PLAIN
        $texts = array(
            'html'  => $this->get_part($mid, "TEXT/HTML"),
            'plain' => $this->get_part($mid, "TEXT/PLAIN")
        );

        // Do I prefer the plain version? If so let's revert the stack
        if($textOnly)
        {
