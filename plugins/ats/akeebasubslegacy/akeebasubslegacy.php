...
class plgAtsAkeebasubslegacy extends JPlugin
{
	...
	public function onAtsHasSubscriptionsComponent()
	{
		if (is_null($this->hasSubscriptionsComponent))
		{
			...
			if ($this->hasSubscriptionsComponent && !class_exists('F0FModel'))
			{
				if (JFolder::exists(JPATH_LIBRARIES . '/f0f'))
				{
					include_once JPATH_LIBRARIES . '/f0f/index.php';
				}

				if (!class_exists('F0FModel'))
				{
					$this->hasSubscriptionsComponent = false;
				}
			}
			...
			{
				$this->hasSubscriptionsComponent = false;
				$model = F0FModel::getTmpInstance('Subscriptions', 'AkeebasubsModel');

				if (is_object($model))
				{
					$class = get_class($model);

					if ($class == 'AkeebasubsModelSubscriptions')
					{
						$this->hasSubscriptionsComponent = true;
					}
				}
			}
		}
