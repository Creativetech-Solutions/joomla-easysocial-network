...
class plgFinderAts extends FinderIndexerAdapter
{
	...
	{
		/** @var  \Akeeba\TicketSystem\Admin\Model\Posts $table */
	...
	public function onFinderAfterSave($context, $row, $isNew)
	{
		...
		if ($context == 'com_ats.post')
		{
			/** @var \Akeeba\TicketSystem\Admin\Model\Posts $row */
	...
	protected function getStateQuery()
	{
		...
		$sql
			// Item ID
			->select('a.ats_post_id AS id')
			// Item and category published state
			->select('a.enabled AS state, c.published AS cat_state')
			// Item and category access levels
			->select('c.access AS cat_access, c.access AS access');

		// Tables to read from
		$sql
			->from($this->table . ' AS a')
			->join('LEFT', '#__ats_tickets AS t ON t.ats_ticket_id = a.ats_ticket_id')
			->join('LEFT', '#__categories AS c ON c.id = t.catid');
